...
def _analysis_autodraft(
    *,
    ...
    # Reference: existing ticket conclusion from comments (avoid dumping all comments).
    comment_body: str | None = None
    comment_created: str | None = None
    comments = _dig(fields, "comment", "comments")
    if isinstance(comments, list):
        for c in reversed(comments):
            ...
            if not isinstance(body, str) or not body.strip():
                continue
            b = body.lower()
            if ("anylysis" in b or "analysis" in b) and ("cpu" in b or "filesystem" in b or "selfheal" in b):
                comment_body = body.strip()
                created = c.get("created")
                comment_created = created if isinstance(created, str) else None
                break

    if comment_body:
        lines.append("")
        lines.append("### Existing conclusion (from Jira comment)")
        if comment_created:
            lines.append(f"- Comment created: {comment_created}")
        lines.append("")
        lines.append("```")
...
def triage(
    ...
    *,
    ...
    try:
        existing = analysis_path.read_text(encoding="utf-8", errors="replace") if analysis_path.exists() else ""
    except Exception:
        existing = ""

    is_autodraft = "Auto-draft." in existing
    if (not analysis_path.exists()) or (not existing.strip()) or (existing.strip() == analysis_template.strip()) or is_autodraft:
        analysis_mode = "wrote_autodraft"
        analysis_md = _analysis_autodraft(
    ...
    try:
        final_analysis_md = analysis_path.read_text(encoding="utf-8", errors="replace")
        analysis_md_len = len(final_analysis_md)
        analysis_txt_path = (ticket_dir / "analysis.txt").resolve()
        analysis_txt_path.write_text(final_analysis_md, encoding="utf-8")
    except Exception:
